...
if (compareToken($_GET['token'], $_GET['user-ip'] . $_GET['user-mac'] . $_GET['timestamp'] . $_GET['date_end'] . $_GET["autowhitelist"])) {
    if (checkTokenAge($_GET['timestamp'])) { // Token has not expired opening firewall
        $cnx = new connexion();

        $was_connected = $cnx->connectUser($_GET['user-mac'], $_GET['user-ip'], $_GET['date_end'], $_GET);
